  ...
  Source(1 to 10)
    ...
    .map{
      case (Success(response), value) =>
        ...
        s"Request $value has recived response: $response"
...
//    .runWith(Sink.foreach[String](println))
  ...
  Source(serverHttpRequests)
    ...
    .runForeach{ //(Try[HttpResponse],String
      ...
      case (Success(response),orderId) =>
        println(s"The order ID $orderId was successful and return the response: $response")
  ...
  //used for high volume + low latency requests
